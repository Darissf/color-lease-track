...
import { Loader2, RotateCw, RotateCcw, EyeOff, Plus, Grid3X3, Layers } from "lucide-react";

type GuideMode = 'none' | 'center' | 'thirds' | 'all';
...
// Landscape canvas for signature images
...
export function SignatureCropper({ file, onCrop, onCancel }: SignatureCropperProps) {
  ...
  // Draw guide lines overlay for landscape canvas
  const drawGuides = (ctx: CanvasRenderingContext2D) => {
    ...
    const thirdX = CANVAS_WIDTH / 3;
    const thirdY = CANVAS_HEIGHT / 3;
    ...
    if (guideMode === 'thirds' || guideMode === 'all') {
      ...
      // Vertical lines
      ctx.moveTo(thirdX, 0);
      ctx.lineTo(thirdX, CANVAS_HEIGHT);
      ctx.moveTo(thirdX * 2, 0);
      ctx.lineTo(thirdX * 2, CANVAS_HEIGHT);
      // Horizontal lines
      ctx.moveTo(0, thirdY);
      ctx.lineTo(CANVAS_WIDTH, thirdY);
      ctx.moveTo(0, thirdY * 2);
      ctx.lineTo(CANVAS_WIDTH, thirdY * 2);
      ctx.stroke();
      
      // Intersection dots (focal points)
      ctx.setLineDash([]);
      ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
      [thirdX, thirdX * 2].forEach(x => {
        [thirdY, thirdY * 2].forEach(y => {
          ctx.beginPath();
  ...
  useEffect(() => {
    ...
    // Calculate draw dimensions to fit in landscape canvas
    ...
    if (rotation === 90 || rotation === 270) {
      ...
      if (rotatedAspectRatio > (CANVAS_WIDTH / CANVAS_HEIGHT)) {
    ...
    const x = centerX + position.x - CANVAS_WIDTH / 2;
    const y = centerY + position.y - CANVAS_HEIGHT / 2;
  ...
  return (
    <Dialog open onOpenChange={onCancel}>
      <DialogContent className="max-w-3xl" aria-describedby="crop-signature-description">
        <DialogHeader>
          <DialogTitle>Crop Tanda Tangan</DialogTitle>
        </DialogHeader>
        <p id="crop-signature-description" className="sr-only">
          Crop dan sesuaikan gambar tanda tangan dengan drag, zoom, dan rotate
        </p>
