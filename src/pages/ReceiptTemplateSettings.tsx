...
import { ArrowLeft, Save, RotateCcw, Palette, Type, Layout, FileText, Settings2, Image as ImageIcon, Loader2, Stamp, PenTool, TextCursor } from 'lucide-react';
...
const ReceiptTemplateSettings = () => {
  ...
  const currentLayoutSettings = settings.receipt_layout_settings;

  // Custom text element handlers
  const handleAddCustomText = async () => {
    try {
      ...
      toast.success('Custom text ditambahkan');
    } catch (error) {
      console.error('Error adding custom text:', error);
      toast.error('Gagal menambahkan custom text');
    }
  };

  const handleUpdateCustomText = async (id: string, updates: Partial<CustomTextElement>) => {
    try {
  ...
  const handleDeleteCustomText = async (id: string) => {
    try {
      ...
      toast.success('Custom text dihapus');
    } catch (error) {
      console.error('Error deleting custom text:', error);
      toast.error('Gagal menghapus custom text');
    }
  };
      ...
      <div className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold">Template Kwitansi</h1>
              <p className="text-xs text-muted-foreground">Kustomisasi dokumen kwitansi</p>
            </div>
          </div>
          ...
          <div className="p-4 space-y-4">
            <Accordion type="multiple" defaultValue={['branding', 'colors', 'custom-text']} className="space-y-3">
              <AccordionItem value="custom-text" className="border rounded-lg px-4">
                <AccordionTrigger className="hover:no-underline py-3"><div className="flex items-center gap-2"><TextCursor className="h-4 w-4 text-primary" /><span className="font-medium">Custom Text Boxes</span></div></AccordionTrigger>
                <AccordionContent className="pb-4">
                  <CustomTextSection
                    ...
                    onUpdate={handleUpdateCustomText}
                    onDelete={handleDeleteCustomText}
              ...
              <AccordionItem value="stamp" className="border rounded-lg px-4">
                ...
                <AccordionContent className="pb-4"><StampSection settings={settings} updateSetting={updateSetting} onFileSelect={handleFileSelect} onRemoveImage={handleRemoveImage} uploading={uploading} activeTab="receipt" layoutSettings={currentLayoutSettings} updateLayoutSetting={updateLayoutSetting} /></AccordionContent>
              </AccordionItem>
              <AccordionItem value="advanced" className="border rounded-lg px-4">
                ...
                <AccordionContent className="pb-4"><AdvancedSection settings={settings} updateSetting={updateSetting} onFileSelect={handleFileSelect} onRemoveImage={handleRemoveImage} uploading={uploading} activeTab="receipt" layoutSettings={currentLayoutSettings} updateLayoutSetting={updateLayoutSetting} /></AccordionContent>
              </AccordionItem>
            </Accordion>
          </div>
        </div>
